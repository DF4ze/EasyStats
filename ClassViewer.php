...
/* 				if (preg_match_all('#class [a-zA-Z0-9_]*(.*?)//php2uml#s', $Fichier, $out)) {
					foreach( $out[0] as $key => $value  ){
						echo "Key : $key = ".$value."<br/><br/><br/><br/>";
				...
				$TempClassTab = get_classes( $Fichier );

				if( $TempClassTab != -1 ){
					foreach( $TempClassTab as $key => $value  ){
						//echo "Key : $key = ".$value['class_complete']."<br/><br/><br/><br/>";
						$ClassTab[$i_class++]['class_complete'] = $value['class_complete'];
					} 
				}else{
					echo "Pas de classe dans ce fichier : $fichier_listed<br/>";
					$nb_file_without_class ++;
	...
	foreach( $ClassTab as $key => $une_class  ){
		...
		if( preg_match('#class ([a-zA-Z0-9_]*)( extends ([a-zA-Z0-9_]*))*#s', $une_class['class_complete'], $out)){
			...
			// echo "<br/><br/>Complete : ".$une_class['class_complete']."<br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/>";
		}
	...
	foreach( $leaves as $key => $nom_feuille  ){
		...
		echo '<div class="classe" style="margin:5px; float:left;">';
			echo '<div class="nom_resum">';
				echo '<h2>Liste "Public" : '.$publics['nom'].'</h2>';
			echo '</div>';
...
// retourne un tableau avec les classes complete retrouvées dans le texte donné.
function get_classes( $text ){
	////////////////////////////////
	// on retrouve la/les balise(s) "class"
	$tab_pos_depart = find_word( 'class', $text );
	
	////////////////////////////////
	// comptage des { et }
	$count = 0;
	$tab_pos_fin = array();
	foreach( $tab_pos_depart as $key => $pos_depart  ){
		// on scan le text
		for( $i=$pos_depart; $i < strlen( $text ) ; $i++ ){
			// si { on compte +1
			if( $text[$i] == '{')
				$count ++;
			// si } on compte -1
				...
				$count --;
				// si le tout fait 0 alors on a fermé autant de } qu'on en a ouvert {
				if( $count == 0 ){
					$tab_pos_fin[] = $i+1; // on enregistre la position //+1 pour prendre }
					$i = strlen( $text ); // on reset pour passer a la classe suivante.
				}
			}
		} 
	} 
	
/* 	foreach( $tab_pos_depart as $key => $value  ){
		echo "depart $key : $value<br/>";
	} 
	foreach( $tab_pos_fin as $key => $value  ){
		echo "fin $key : $value<br/>";
	} 
	 */
	
	//////////////////////////////////
	// on récupère le texte entre ceci.
	$tab_class = array();
	foreach( $tab_pos_depart as $key => $pos_depart  ){
		$tab_class[]['class_complete'] = substr( $text, $pos_depart, $tab_pos_fin[$key] - $pos_depart ); 
	} 
	
	if( count($tab_pos_depart) == 0 )
		return -1;
	else
		return $tab_class;
}
// retourne un tableau de position du 'word' dans le 'text'
// $tab[x] = position dans le texte (à la strpos).
function find_word( $word, $text ){
	$len_word = strlen( " $word " ); 
	$pile = new pile( $len_word ); // on fait une pile de la taille du mot + un espace de chaque coté : etre sur qu'il n'y ait pas de caractère collé au mot .. et donc ca ne serait aps le mot qu'on cherche rééllement.

	$tab_offset = array();
	// On fait le tour du texte
	for( $i=0; $i < strlen( $text ); $i++ ){
		// On contactène caractère par caractère.
		$pile->add( $text[$i] );
		//echo 'pile : '.$i.'.'.trim($pile).' <br/>';
			
		// Le mot commence bien par $word
		if( trim($pile) == $word ){
			// il faut vérifier le caractère apres ... que ca soit bien un ' '.
			if( $text[$i+1] == ' '){
				$tab_offset[] = $i - $len_word +1; // strlen(' word') et non strlen(' word ')
				$pile->clear(); // pour éviter de compter 2 fois le meme.
			}
		}	
	} 
